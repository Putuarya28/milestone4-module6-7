...
describe('AuthController', () => {
  ...
  describe('register', () => {
    it('should register a user', async () => {
      mockService.register.mockResolvedValue({ id: 1, email: 'test@example.com' });
    ...
    it('should throw 400 on invalid input', async () => {
      mockService.register.mockImplementation(() => { throw { status: 400, message: 'Bad Request' }; });
      await expect(controller.register({} as any)).rejects.toMatchObject({ status: 400 });
    });
  });
  ...
  describe('login', () => {
    ...
    it('should throw 401 on invalid credentials', async () => {
      mockService.login.mockImplementation(() => { throw { status: 401, message: 'Unauthorized' }; });
      await expect(controller.login({ email: 'bad', password: 'bad' })).rejects.toMatchObject({ status: 401 });
    });
  });
});
